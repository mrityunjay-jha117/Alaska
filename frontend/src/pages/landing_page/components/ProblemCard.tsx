...
export function ProblemCard({ title, desc }: { title: string; desc: string }) {
  return (
    <motion.div variants={{ hidden: { opacity: 0, x: -20 }, visible: { opacity: 1, x: 0 } }} className="flex gap-6 items-start group">
      <div className="mt-1 w-14 h-14 rounded-2xl bg-white/70 backdrop-blur-xl border border-white/40 text-foreground/50 flex items-center justify-center shrink-0 font-bold group-hover:bg-primary/10 group-hover:text-primary group-hover:border-primary/30 transition-all shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
        <X size={24} />
      </div>
      <div>
